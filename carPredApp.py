...
if nav == "Prediction":
    ...
    #st.markdown(f"Brand ENC : {brand_enc} ")
    #st.markdown(f"Model ENC : {model_enc} ")
    #st.markdown(f"seller ENC : {seller_type_enc} ")
    #st.markdown(f"fuel ENC : {fuel_type_enc} ")
    #st.markdown(f"transmission ENC : {transmission_type_enc} ")
    ...
    #st.markdown("""<br><br>Test it!!""",unsafe_allow_html=True)
    #st.dataframe(carsX[1:2])
    #st.dataframe(cars['selling_price'][1:2])
